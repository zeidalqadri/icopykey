...
``normalize(data: bytes, strict: bool = True) -> MifareClassicDump``
    ...
    ``strict`` is ``True`` strategies must raise an exception on any
...
class RawFormatStrategy(FormatStrategy):
    ...
    def can_handle(self, data: bytes) -> bool:
        # Accept any buffer at least one block long and aligned to 16 bytes.
    ...
    def normalize(self, data: bytes, strict: bool = True) -> 'MifareClassicDump':
        ...
        # Extract UID from manufacturer block (first 4 bytes)
        ...
        from .engine import MifareClassicDump  # local import to avoid cycle
    ...
    def _extract_keys(self, data: bytes) -> List[Tuple[Optional[str], Optional[str]]]:
        ...
        if len(data) == 1024:
            ...
            # fallback: treat as sequence of 4‑block sectors
        ...
        for size in sector_sizes:
            ...
            if len(block) < 16:
                ...
                # If key appears default (all 00 or all FF), treat as unknown
                if key_a == "000000000000" or key_a == "ffffffffffff":
                    key_a = None
                if key_b == "000000000000" or key_b == "ffffffffffff":
                    key_b = None
...
class X100FormatStrategy(FormatStrategy):
    ...
    def normalize(self, data: bytes, strict: bool = True) -> 'MifareClassicDump':
        ...
        if len(payload) % 16 != 0:
            ...
            payload += b"\x00" * missing
        uid: Optional[str] = None
        # Use RawFormatStrategy for key extraction
